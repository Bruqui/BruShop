                ...
                <div className='flex gap-10 flex-col xl:flex-row rounded-2xl p-3 mb-6'>
                    ...
                    <div className='flex flex-1 gap-x-2 max-w-[477px]'>
                        <div className='flex-1 flexCenter flex-col gap-[7px] flex-wrap'>
                            {product.image.map((item, i) => (
                                <img onClick={() => setImage(item)} src={item} alt="prdcImg" className='object-cover aspect-square rounded-lg' />
                            ))}
                        </div>
                        <div className='flex-[4] flex'>
                            <img src={image} alt="prdcImg" className='rounded-xl' />
                        </div>
                    </div>
                    {/** PRODUCT INFO */}
                    <div className='flex-[1.5] rounded-2xl px-5 py-3 bg-primary'>
                        ...
                        <div className='flex flex-col gap-4 my-4 mb-5'>
                            <div className='flex gap-2'>
                                {[...product.colors].map((item, i) => (
                                    <button key={i} onClick={() => setColor(item)} className={`h-9 w-9 rounded-full flexCenter`} style={{ background: item }}>
                        ...
                        <div className='mt-2 flex flex-col gap-1 text-gray-30 text-[14px]'>
                            ...
                            <p>Easy Returns ans Exchanges Within 7 Days</p>
                        </div>
                    </div>
                </div>
                <ProductDescription />
