        ...
        <div className='px-2 sm:px-8 mt-2 sm:mt-14 pb-16'>
            <form onSubmit={onSubmitHandler} className='flex flex-col gap-y-3 medium-14 lg:w-[777px]'>
                ...
                <div className='w-full'>
                    ...
                    <textarea onChange={(e) => setDescription(e.target.value)} value={description} rows={5} placeholder='Write here...' className='px-3 py-1.5 ring-1 ring-slate-900/10 rounded bg-white mt-1 w-full max-w-lg' />
                </div>
                {/** CATEGORIES */}
                ...
                <div className='flex gap-2 pt-2'>
                    {["image1", "image2", "image3", "image4"].map((imgKey, i) => (
                        <label key={i} htmlFor={imgKey}>
                            <img src={images[imgKey] ? URL.createObjectURL(images[imgKey]) : upload_icon} alt="" className='w-16 h-16 aspect-square object-cover ring-1 ring-slate-900/5 rounded-lg' />
